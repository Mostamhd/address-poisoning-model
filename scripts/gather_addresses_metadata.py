...
def get_address_erc20_transactions(address_id):
    ...
    query = "SELECT tx_id FROM erc20_transfer WHERE address_id = %s;"
    cursor.execute(query, (address_id,))
...
def print_progress(index, total, num_transactions, address):
    ...
    print(
        f"Processing address {index + 1}/{total} ({progress:.2f}%), Address: {address}, Transactions: {num_transactions}"
    )
        ...
        for index in range(last_index, total_addresses):
            ...
            if address_id:
                ...
                for erc20_transactions_tx in erc20_transactions:
                    erc20_transaction_trasnfers = get_tx_erc20_transfers(
                        erc20_transactions_tx["tx_id"]
                    )

                    main_tx_creator = get_erc20_transfers_tx_creator(
                        erc20_transactions_tx["tx_id"]
                    )
                    main_tx_reciever = get_erc20_transfers_tx_reciever(
                        erc20_transactions_tx["tx_id"]
                    )
                    ...
                    for tx in erc20_transaction_trasnfers:
                        tx_metadata = {
                            ...
                            "total_supply": tx["total_supply"].decode("utf-8")
                            if tx["total_supply"]
                            else "",
                        ...
                        output_file.write(
                            ",".join(map(str, tx_metadata.values())) + "\n"
                        )
            else:
